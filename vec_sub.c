Datum vec_sub_with_vec(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_sub_with_vec);

/**
 * Does element-wise subtraction between two vectors.
 *
 ...
 */
Datum
vec_sub_with_vec(PG_FUNCTION_ARGS)
{
  Oid elemTypeId;
  int16 elemTypeWidth;
  ...
  if (elemTypeId != ARR_ELEMTYPE(rhsArray)) {
    ereport(ERROR, (errmsg("vec_sub input arrays must be the same type")));
  }
  ...
  for (i = 0; i < lhsLength; i++) {
    ...
    switch(elemTypeId) {
      ...
      case NUMERICOID:
        ...
        retContent[i] = DirectFunctionCall2(numeric_sub, lhsContent[i], rhsContent[i]);
#else
        retContent[i] = NumericGetDatum(numeric_sub_opt_error(DatumGetNumeric(lhsContent[i]), DatumGetNumeric(rhsContent[i]), NULL));
#endif
        break;
    }
  }
...
Datum vec_sub_with_scalar(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_sub_with_scalar);

/**
 * Subtracts a scalar to/from all elements of a given vector.
 *
 ...
 */
Datum
vec_sub_with_scalar(PG_FUNCTION_ARGS)
{
  ...
  if (elemTypeId1 == INT2OID ||
    ...
    ereport(ERROR, (errmsg("vec_sub scalar operand must be a numeric type")));
  }
  ...
  if (ARR_ELEMTYPE(inputArray) != scalarTypeId) {
    ereport(ERROR, (errmsg("vec_sub array elements and scalar operand must be the same type")));
  }
  ...
  for (i = 0; i < inputLength; i++) {
    ...
    if (scalarPos == 0) {
      switch(scalarTypeId) {
        ...
        case NUMERICOID:
#if PG_VERSION_NUM < 120000
          retContent[i] = DirectFunctionCall2(numeric_sub, NumericGetDatum(scalar.num), inputContent[i]);
#else
          retContent[i] = NumericGetDatum(numeric_sub_opt_error(scalar.num, DatumGetNumeric(inputContent[i]), NULL));
#endif
          break;
      }
    } else {
      switch(scalarTypeId) {
        ...
        case NUMERICOID:
#if PG_VERSION_NUM < 120000
          retContent[i] = DirectFunctionCall2(numeric_sub, inputContent[i], NumericGetDatum(scalar.num));
#else
          retContent[i] = NumericGetDatum(numeric_sub_opt_error(DatumGetNumeric(inputContent[i]), scalar.num, NULL));
#endif
          break;
      }
    }
