Datum vec_div_with_vec(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_div_with_vec);

/**
 * Does element-wise division between two vectors.
 *
 ...
 */
Datum
vec_div_with_vec(PG_FUNCTION_ARGS)
{
  Oid elemTypeId;
  int16 elemTypeWidth;
  ...
  if (elemTypeId != ARR_ELEMTYPE(rhsArray)) {
    ereport(ERROR, (errmsg("vec_div input arrays must be the same type")));
  }
  ...
  for (i = 0; i < lhsLength; i++) {
    ...
    switch(elemTypeId) {
      ...
      case FLOAT8OID:
        ...
        break;
      case NUMERICOID:
        if (DatumGetInt32(DirectFunctionCall2(numeric_cmp, rhsContent[i], NUMERIC_ZERO)) == 0) {
#if PG_VERSION_NUM < 140000
          retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("NaN"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
#else
          retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("Infinity"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
#endif
        } else {
#if PG_VERSION_NUM < 120000
          retContent[i] = DirectFunctionCall2(numeric_div, lhsContent[i], rhsContent[i]);
#else
          retContent[i] = NumericGetDatum(numeric_div_opt_error(DatumGetNumeric(lhsContent[i]), DatumGetNumeric(rhsContent[i]), NULL));
#endif
        }
        break;
    }
...
Datum vec_div_with_scalar(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_div_with_scalar);

/**
 * Divides a scalar by/into all elements of a given vector.
 *
 ...
 */
Datum
vec_div_with_scalar(PG_FUNCTION_ARGS)
{
  ...
  if (elemTypeId1 == INT2OID ||
    ...
    ereport(ERROR, (errmsg("vec_div scalar operand must be a numeric type")));
  }
  ...
  if (ARR_ELEMTYPE(inputArray) != scalarTypeId) {
    ereport(ERROR, (errmsg("vec_div array elements and scalar operand must be the same type")));
  }
    ...
    if (scalarPos == 0) {
      switch(scalarTypeId) {
        ...
        case FLOAT8OID:
          ...
          break;
        case NUMERICOID:
          if (DatumGetInt32(DirectFunctionCall2(numeric_cmp, inputContent[i], NUMERIC_ZERO)) == 0) {
#if PG_VERSION_NUM < 140000
            retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("NaN"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
#else
            retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("Infinity"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
...
#if PG_VERSION_NUM < 120000
            retContent[i] = DirectFunctionCall2(numeric_div, NumericGetDatum(scalar.num), inputContent[i]);
#else
            retContent[i] = NumericGetDatum(numeric_div_opt_error(scalar.num, DatumGetNumeric(inputContent[i]), NULL));
#endif
          }
          break;
      }
      ...
      switch(scalarTypeId) {
        case INT2OID:
          retContent[i] = Int16GetDatum(DatumGetInt16(inputContent[i]) / scalar.i16);
          break;
        case INT4OID:
          retContent[i] = Int32GetDatum(DatumGetInt32(inputContent[i]) / scalar.i32);
          break;
        case INT8OID:
          retContent[i] = Int64GetDatum(DatumGetInt64(inputContent[i]) / scalar.i64);
          break;
        case FLOAT4OID:
          retContent[i] = Float4GetDatum(DatumGetFloat4(inputContent[i]) / scalar.f4);
          break;
        case FLOAT8OID:
          retContent[i] = Float8GetDatum(DatumGetFloat8(inputContent[i]) / scalar.f8);
          break;
        case NUMERICOID:
          if (DatumGetInt32(DirectFunctionCall2(numeric_cmp, NumericGetDatum(scalar.num), NUMERIC_ZERO)) == 0) {
#if PG_VERSION_NUM < 140000
            retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("NaN"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
#else
            retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("Infinity"), ObjectIdGetDatum(InvalidOid), Int32GetDatum(-1));
...
#if PG_VERSION_NUM < 120000
            retContent[i] = DirectFunctionCall2(numeric_div, inputContent[i], NumericGetDatum(scalar.num));
#else
            retContent[i] = NumericGetDatum(numeric_div_opt_error(DatumGetNumeric(inputContent[i]), scalar.num, NULL));
#endif
          }
          break;
      }
